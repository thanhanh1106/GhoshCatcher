...
namespace Unicorn
{
    ...
    public class PlayerDataManager : MonoBehaviour, IDataSkin
    {
        ...
        // kiểm tra lần đầu vào game 
        public bool IsEnteredGame
        {
            get
            {
                if(PlayerPrefs.GetInt(GameConst.ENTERED_GAME, 0) == 0)
                {
                    PlayerPrefs.SetInt(GameConst.ENTERED_GAME, 1);
                    return false;
                } 
                return true;
            }
        ...
        {
            return ES3.Load<List<DataLevel>>(GameConst.DATA_LEVEL);
        }
        public void SaveDataLevel(List<DataLevel> datas)
        {
            ES3.Save(GameConst.DATA_LEVEL, datas);
        }
